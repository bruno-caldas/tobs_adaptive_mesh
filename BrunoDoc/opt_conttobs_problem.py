...
class OP(AProb.AP):
    ...
    def __vf_fun_var_assem2__(self):
        ...
        self.malhavol_vol_xi2  = assemble(rho_tst2 * Constant(1) * dx)
    ...
    def obj_fun(self, rho, user_data=None):
        ...
        funcional1, w = self.Funcional(ds_vars)
        ...
        print("Pressao: {}".format(borda_e))

        if self.first_iter:
            self.file_obj.write("FunObj" +"\t"+ "Pressao"+ "\n")
            self.first_iter = False
        self.file_obj.write(str(fval) +"\t"+ str(borda_e)  + "\n")
        self.file_obj.close()
        return fval
    ...
    def obj_dfun(self, xi, user_data=None):
        ...
        self.file_analise.write(str(fval1) + '\t'+ str(L.sum()) + '\n')
    ...
    def volfrac_fun2(self, xi):
        ...
        volume_val2 = numpy.array(assemble(self.rho_tst * self.rho* dx)).sum()
